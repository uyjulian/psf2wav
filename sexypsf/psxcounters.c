...
void psxRcntUpdate()
{
    ...
    if( cycle - rcnts[3].cycleStart >= rcnts[3].cycle )
    {
        ...
        // Update spu.
        if( spuSyncCount >= SpuUpdInterval[/* Config.PsxType */ 0] )
        {
            spuSyncCount = 0;
...
#if 0
        // VSync irq.
        if( hSyncCount == VBlankStart[/* Config.PsxType */ 0] )
        {
            GPU_vBlank( 1 );
        ...
        if( hSyncCount >= HSyncTotal[/* Config.PsxType */ 0] )
        {
            hSyncCount = 0;
...
void psxRcntWmode( u32 index, u32 value )
{
    ...
    switch( index )
    {
        ...
        case 1:
            ...
            {
                rcnts[index].rate = (PSXCLK / (FrameRate[/* Config.PsxType */ 0] * HSyncTotal[/* Config.PsxType */ 0]));
            }
            else
            {
                rcnts[index].rate = 1;
...
#if 0
    if (Config.VSyncWA) {
        HSyncTotal[/* Config.PsxType */ 0] = HSyncTotal[/* Config.PsxType */ 0] / BIAS;
    } else if (Config.HackFix) {
        HSyncTotal[/* Config.PsxType */ 0] = HSyncTotal[/* Config.PsxType */ 0]+1;
    }
#endif
}
...
void psxRcntInit()
{
    ...
    rcnts[3].target = (PSXCLK / (FrameRate[/* Config.PsxType */ 0] * HSyncTotal[/* Config.PsxType */ 0]));
...
void CounterDeadLoopSkip()
{
    ...
    for(x=0;x<4;x++)
    {
     if (rcnts[x].cycle != 0xffffffff)
     {
      min=rcnts[x].cycle;
      min-=(psxRegs.cycle - rcnts[x].cycleStart);
